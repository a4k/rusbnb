...
class RoomPhoto(Resource):
    ...
    def post(self, room_id):
        ...
        if photo_title is None:
            return {"error": "photo title is requirement"}, HTTPStatus.BAD_REQUEST
        if photo_description is None:
            return {"error": "photo description is requirement"}, HTTPStatus.BAD_REQUEST
        if photo_file is None:
            return {"error": "photo file is requirement"}, HTTPStatus.BAD_REQUEST
        ...
        if photo_extension not in self.allowed_photo_extensions:
            photo_extension = 'png'
        ...
        with Image.open(photo_obj) as photo_image:
            # and save it in folder room-images with name that is id of photo object and allowed extension
            photo_image.save('room-images/{}.{}'.format(photo_obj.id, photo_extension))

        return {"message": "Successfully created photo"}, HTTPStatus.ACCEPTED
